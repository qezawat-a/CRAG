...
2. What the script does: clone/update the TradeKit repository to `~/.xt-tradekit/app` â†’ create a Python venv and install dependencies â†’ `claude mcp add --scope user` to register the stdio server (if the claude CLI is not detected, it prints the manual configuration JSON for each client).
